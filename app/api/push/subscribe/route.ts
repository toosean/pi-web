import { addSubscription, getVapidPublicKey, removeSubscription, type PushSubscriptionRecord } from "@/lib/web-push";
...
// GET /api/push/subscribe - expose VAPID public key
export async function GET(): Promise<Response> {
  try {
    const publicKey = await getVapidPublicKey();
    return Response.json({ publicKey, vapidPublicKey: publicKey });
  } catch (error) {
...
export async function POST(req: Request): Promise<Response> {
  ...
  const locale = body.locale === "zh-CN" ? "zh-CN" : (body.locale === "zh-TW" ? "zh-TW" : "en");
