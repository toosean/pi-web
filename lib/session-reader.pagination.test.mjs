...
test("alignToTurn: long single turn (80 entries > tail 50) is never cut mid-turn", () => {
  // Turn structure: 1 user message followed by 79 toolCall / assistant steps.
  const entries = [
    { id: "e0", parentId: null, type: "message", timestamp: "t0", message: { role: "user", content: "perform heavy task" } },
  ];
  for (let i = 1; i <= 79; i++) {
    entries.push({
      id: `e${i}`,
  ...
  // With default tail = 50, slicing must NOT cut at 50; it must include the entire turn up to e0.
  const ctx = buildSessionContext(entries, "e79", { tail: 50, alignToTurn: true });
  assert.equal(ctx.messages.length, 80);
  assert.equal(ctx.entryIds[0], "e0");
  assert.equal(ctx.entryIds[ctx.entryIds.length - 1], "e79");
  assert.equal(ctx.hasMore, false);
});

test("alignToTurn: multi-turn session aligns cleanly to user message boundary", () => {
  // Turn 1: u1 -> a1_1 -> a1_2 (3 entries)
  // Turn 2: u2 -> 70 assistant steps (71 entries)
  const entries = [
    { id: "u1", parentId: null, type: "message", timestamp: "t0", message: { role: "user", content: "turn 1 prompt" } },
    { id: "a1_1", parentId: "u1", type: "message", timestamp: "t1", message: { role: "assistant", content: "turn 1 step 1" } },
    { id: "a1_2", parentId: "a1_1", type: "message", timestamp: "t2", message: { role: "assistant", content: "turn 1 done" } },
    { id: "u2", parentId: "a1_2", type: "message", timestamp: "t3", message: { role: "user", content: "turn 2 prompt" } },
  ];
  for (let i = 1; i <= 70; i++) {
    entries.push({
      id: `a2_${i}`,
      parentId: i === 1 ? "u2" : `a2_${i - 1}`,
      type: "message",
      timestamp: `t3_${i}`,
      message: { role: "assistant", content: `turn 2 step ${i}` },
    });
  }

  // Page 1: loading active leaf (a2_70) loads the full turn 2 (71 entries), aligned to u2.
  const page1 = buildSessionContext(entries, "a2_70", { tail: 50, alignToTurn: true });
  assert.equal(page1.entryIds[0], "u2");
  assert.equal(page1.entryIds[page1.entryIds.length - 1], "a2_70");
  assert.equal(page1.entryIds.length, 71);
  assert.equal(page1.hasMore, true);

  // Page 2: paging upward before u2 loads turn 1 cleanly up to root without duplicates.
  const page2 = buildSessionContext(entries, page1.oldestEntryId, { tail: 50, excludeLeaf: true, alignToTurn: true });
  assert.deepEqual(page2.entryIds, ["u1", "a1_1", "a1_2"]);
  assert.equal(page2.hasMore, false);
  assert.ok(page1.entryIds.every((id) => !page2.entryIds.includes(id)));
});

test("alignToTurn: maxTurns: 1 loads exactly one complete turn", () => {
  const entries = linearChain(20);
  const ctx = buildSessionContext(entries, "e19", { alignToTurn: true, maxTurns: 1 });
  // e19 is assistant, e18 is user -> 1 complete turn is [e18, e19]
  assert.deepEqual(ctx.entryIds, ["e18", "e19"]);
  assert.equal(ctx.hasMore, true);
});

test("alignToTurn: continues upward turn-by-turn until reaching at least 50 entries, then stops cleanly at user message", () => {
  // Construct 4 turns:
  // Turn 1 (oldest): u1 + 10 assistants = 11 entries
  // Turn 2: u2 + 20 assistants = 21 entries
  // Turn 3: u3 + 20 assistants = 21 entries
  // Turn 4 (latest): u4 + 10 assistants = 11 entries
  const entries = [
    { id: "u1", parentId: null, type: "message", timestamp: "t1_0", message: { role: "user", content: "t1 user" } },
  ];
  let lastId = "u1";
  for (let i = 1; i <= 10; i++) {
    const id = `a1_${i}`;
    entries.push({ id, parentId: lastId, type: "message", timestamp: `t1_${i}`, message: { role: "assistant", content: `t1 a ${i}` } });
    lastId = id;
  }

  entries.push({ id: "u2", parentId: lastId, type: "message", timestamp: "t2_0", message: { role: "user", content: "t2 user" } });
  lastId = "u2";
  for (let i = 1; i <= 20; i++) {
    const id = `a2_${i}`;
    entries.push({ id, parentId: lastId, type: "message", timestamp: `t2_${i}`, message: { role: "assistant", content: `t2 a ${i}` } });
    lastId = id;
  }

  entries.push({ id: "u3", parentId: lastId, type: "message", timestamp: "t3_0", message: { role: "user", content: "t3 user" } });
  lastId = "u3";
  for (let i = 1; i <= 20; i++) {
    const id = `a3_${i}`;
    entries.push({ id, parentId: lastId, type: "message", timestamp: `t3_${i}`, message: { role: "assistant", content: `t3 a ${i}` } });
    lastId = id;
  }

  entries.push({ id: "u4", parentId: lastId, type: "message", timestamp: "t4_0", message: { role: "user", content: "t4 user" } });
  lastId = "u4";
  for (let i = 1; i <= 10; i++) {
    const id = `a4_${i}`;
    entries.push({ id, parentId: lastId, type: "message", timestamp: `t4_${i}`, message: { role: "assistant", content: `t4 a ${i}` } });
    lastId = id;
  }

  // From active leaf a4_10:
  // Turn 4 has 11 entries (< 50) -> continues to Turn 3 (11 + 21 = 32 < 50) -> continues to Turn 2 (32 + 21 = 53 >= 50).
  // Stops at u2!
  const ctx = buildSessionContext(entries, "a4_10", { tail: 50, alignToTurn: true });
  assert.equal(ctx.entryIds[0], "u2");
  assert.equal(ctx.entryIds[ctx.entryIds.length - 1], "a4_10");
  assert.equal(ctx.entryIds.length, 53);
  assert.equal(ctx.hasMore, true);

  // Turn 1 was not loaded yet; paging upward before u2 loads Turn 1 cleanly.
  const page2 = buildSessionContext(entries, ctx.oldestEntryId, { tail: 50, excludeLeaf: true, alignToTurn: true });
  assert.equal(page2.entryIds[0], "u1");
  assert.equal(page2.entryIds[page2.entryIds.length - 1], "a1_10");
  assert.equal(page2.entryIds.length, 11);
  assert.equal(page2.hasMore, false);
});
