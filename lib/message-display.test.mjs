...
test("collects session modified files from file-modifying toolCalls", async () => {
  const { collectSessionModifiedFiles } = await loadSubject();
  const messages = [
    { role: "user", content: "Fix the bug" },
    assistant([
      { type: "toolCall", toolCallId: "c1", toolName: "edit", input: { path: "src/app.ts" } },
      { type: "toolCall", toolCallId: "c2", toolName: "write", input: { filePath: "src/utils.ts" } },
      { type: "toolCall", toolCallId: "c3", toolName: "read", input: { path: "package.json" } },
      { type: "toolCall", toolCallId: "c4", toolName: "edit", input: { path: "src/app.ts" } },
    ]),
  ];

  const files = collectSessionModifiedFiles(messages);
  assert.deepEqual(files, ["src/app.ts", "src/utils.ts"]);
});

test("collects modified files strictly per turn when messages slice is scoped", async () => {
  const { collectSessionModifiedFiles } = await loadSubject();
  const turn1Messages = [
    { role: "user", content: "Turn 1" },
    assistant([
      { type: "toolCall", toolCallId: "c1", toolName: "edit", input: { path: "src/turn1.ts" } },
    ]),
  ];
  const turn2Messages = [
    { role: "user", content: "Turn 2" },
    assistant([
      { type: "toolCall", toolCallId: "c2", toolName: "write", input: { filePath: "src/turn2.ts" } },
    ]),
  ];

  assert.deepEqual(collectSessionModifiedFiles(turn1Messages), ["src/turn1.ts"]);
  assert.deepEqual(collectSessionModifiedFiles(turn2Messages), ["src/turn2.ts"]);
});
