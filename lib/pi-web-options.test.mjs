...
test("supports the allow-all-files CLI option and environment variable", () => {
  assert.equal(parseLaunchOptions(["--allow-all-files"], {}).allowAllFiles, true);
  for (const value of ["1", "true", "TRUE", "yes", "on"]) {
    assert.equal(parseLaunchOptions([], { PI_WEB_ALLOW_ALL_FILES: value }).allowAllFiles, true);
    assert.equal(parseLaunchOptions([], { PI_WEB_DISABLE_FILE_SECURITY: value }).allowAllFiles, true);
  }
  for (const value of ["0", "false", "off", ""]) {
    assert.equal(parseLaunchOptions([], { PI_WEB_ALLOW_ALL_FILES: value }).allowAllFiles, false);
    assert.equal(parseLaunchOptions([], { PI_WEB_DISABLE_FILE_SECURITY: value }).allowAllFiles, false);
  }
});
