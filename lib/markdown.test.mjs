...
import { normalizeDisplayMath, replaceLocalhostUrl } from "./markdown.ts";
...
describe("normalizeDisplayMath", () => {
  ...
  describe("replaceLocalhostUrl", () => {
    it("does not replace URLs if targetBase / NEXT_PUBLIC_REPLACEMENT_BASE_URL is unconfigured", () => {
      const saved = process.env.NEXT_PUBLIC_REPLACEMENT_BASE_URL;
      delete process.env.NEXT_PUBLIC_REPLACEMENT_BASE_URL;
      try {
        assert.equal(replaceLocalhostUrl("http://127.0.0.1:3000"), "http://127.0.0.1:3000");
        assert.equal(replaceLocalhostUrl("http://localhost:8080/path"), "http://localhost:8080/path");
      } finally {
        if (saved !== undefined) process.env.NEXT_PUBLIC_REPLACEMENT_BASE_URL = saved;
      }
    });

    it("replaces http://127.0.0.1 and http://localhost when targetBase is specified", () => {
      assert.equal(replaceLocalhostUrl("http://127.0.0.1", "http://mydomain"), "http://mydomain");
      assert.equal(replaceLocalhostUrl("http://localhost", "http://mydomain"), "http://mydomain");
      assert.equal(replaceLocalhostUrl("http://127.0.0.1:3000/api/test", "http://mydomain"), "http://mydomain:3000/api/test");
      assert.equal(replaceLocalhostUrl("http://localhost:8080/path", "http://mydomain"), "http://mydomain:8080/path");
    });

    it("accepts a custom base URL argument", () => {
      assert.equal(replaceLocalhostUrl("http://127.0.0.1:8000", "https://custom.org"), "https://custom.org:8000");
    });

    it("leaves other URLs unchanged", () => {
      assert.equal(replaceLocalhostUrl("https://example.com/api", "http://mydomain"), "https://example.com/api");
      assert.equal(replaceLocalhostUrl("http://192.168.1.1/test", "http://mydomain"), "http://192.168.1.1/test");
    });
  });
});
