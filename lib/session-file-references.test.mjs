...
test("resolves relative path references against session cwd", async () => {
  const { isFilePathReferencedByEntries } = await loadSubject();
  const entries = [
    {
      type: "session",
  ...
  assert.equal(isFilePathReferencedByEntries("/workspace/my-app/assets/logo.png", entries), true);
});
