...
test("findDraftWindowFor predicts the id openDraftWindow resolves to", () => {
  const first = openDraftWindow([], { windowId: "d1", draftKey: "new:d1:/repo", cwd: "/repo", now: 10 });

  assert.equal(findDraftWindowFor(first.windows, "new:d1:/repo", "/repo")?.windowId, "d1");
  // A different draft key for a cwd that already has a parked composer resolves
  // to that composer. A caller that activates a freshly generated id instead
  // leaves activeWindowId pointing at a window that was never inserted.
  assert.equal(findDraftWindowFor(first.windows, "new:d2:/repo", "/repo")?.windowId, "d1");
  const opened = openDraftWindow(first.windows, {
    windowId: "d2",
    draftKey: "new:d2:/repo",
    cwd: "/repo",
    now: 20,
  });
  assert.equal(opened.windowId, "d1");
  assert.equal(opened.windows.length, 1);

  // No parked composer for this cwd: the caller's own id is the one that lands.
  assert.equal(findDraftWindowFor(first.windows, "new:d3:/other", "/other"), null);
  const fresh = openDraftWindow(first.windows, {
    windowId: "d3",
    draftKey: "new:d3:/other",
    cwd: "/other",
    now: 30,
  });
  assert.equal(fresh.windowId, "d3");
});
