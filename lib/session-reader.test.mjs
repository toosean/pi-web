...
const {
  ...
  resolveSessionPath,
  setSessionCatalogLoaderForTesting,
} = await jiti.import("./session-reader.ts");
const { SessionManager } = await jiti.import("@earendil-works/pi-coding-agent");

/**
 * Replaces the file→metadata catalogue loader for a test. These tests care about
 * how the catalogue is consumed (dedup, invalidation, merge, targeted lookup),
 * not about how it is scanned, so they hand `listAllSessions()` a synthetic one.
 * `lib/session-metadata.test.mjs` covers the scanner itself.
 */
function stubSessionCatalog(entriesOrLoader) {
  setSessionCatalogLoaderForTesting(
    typeof entriesOrLoader === "function" ? entriesOrLoader : async () => entriesOrLoader,
  );
}
...
test("session listing reads subagent relations and terminal status without reopening full session files", async (t) => {
  ...
  resetSessionListState();
  t.after(() => {
    setSessionCatalogLoaderForTesting(null);
