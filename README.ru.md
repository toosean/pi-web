...
| `PI_WEB_TOOL_INPUT_FORMAT` | Формат аргументов инструмента в развёрнутом виде: `json` или `yaml` | `json` |
| `PI_WEB_ALLOW_ALL_FILES` | Разрешить просмотр файлов и worktree доступ к любому файлу | Не задано |
| `PI_WEB_IDLE_TIMEOUT_MS` | Тайм-аут простоя сессии в миллисекундах; `0` отключает завершение | `600000` (10 мин) |
| `NEXT_PUBLIC_REPLACEMENT_BASE_URL` | Заменять localhost-ссылки Markdown на этот внешний базовый URL | Не задано |
| `PI_WEB_DIST_DIR` | Каталог сборки, позволяющий не перезаписывать рабочий `.next` | `.next` |
