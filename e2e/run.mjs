...
try {
  ...
  server = spawn(process.execPath, [join(root, "node_modules/next/dist/bin/next"), mode, "-H", "127.0.0.1", "-p", String(port)], {
    ...
    env: {
      ...
      PI_WEB_PASSWORD: "",
      PI_WEB_ALLOW_ALL_FILES: "false",
      PI_WEB_DISABLE_FILE_SECURITY: "false",
  ...
  for (const viewport of [{ width: 1280, height: 800 }, { width: 390, height: 844 }]) {
    ...
    if (viewport.width > 600) {
      ...
      const waitForReadingOffset = (target, expected) => target.evaluate((element, expectedOffset) => new Promise((resolve, reject) => {
        const deadline = performance.now() + 10_000;
        const check = () => {
          const container = element.closest(".overflow-y-auto");
          const actual = element.getBoundingClientRect().top - container.getBoundingClientRect().top;
          if (Math.abs(actual - expectedOffset) < 5) return resolve();
          if (performance.now() >= deadline) return reject(new Error(`Reading offset ${actual} did not restore to ${expectedOffset}`));
      ...
      await page.locator("[data-session-window][aria-hidden='false']")
        .getByText("Scroll up to load earlier messages", { exact: true })
      ...
      try {
        ...
        await page.locator("[data-session-window][aria-hidden='false'] [data-entry-id='e4920']").waitFor();
