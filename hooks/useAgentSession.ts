...
import { getCachedSession, cacheSession, invalidateSessionCache, mergeRevalidatedSessionData } from "@/lib/session-cache";
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const sessionRunning = opts.sessionRunning ?? false;
  ...
  const windowActivationAnchorRef = useRef<{ entryId: string; offset: number } | null>(null);
  ...
  const loadSession = useCallback(async (
    ...
    try {
      ...
      const freshData = await res.json() as SessionData;
      if (sessionIdRef.current !== sid) return null;
      const d = preserveLoadedHistory
        ? mergeRevalidatedSessionData(dataRef.current, freshData)
        : freshData;
  ...
  useEffect(() => {
    ...
    const renewLease = async () => {
      ...
      try {
        ...
        await response.json() as { renewed?: number };
      } catch {
  ...
  useLayoutEffect(() => {
    if (!isActive) return;
    const container = scrollContainerRef.current;
    const anchor = windowActivationAnchorRef.current;
    if (container && anchor) {
      const element = Array.from(container.querySelectorAll<HTMLElement>("[data-entry-id]:not([data-message-role])"))
        .find((candidate) => candidate.dataset.entryId === anchor.entryId);
      if (element) {
    ...
    return () => {
      ...
      const element = Array.from(current.querySelectorAll<HTMLElement>("[data-entry-id]:not([data-message-role])"))
        .find((candidate) => candidate.getBoundingClientRect().bottom > viewportTop);
      windowActivationAnchorRef.current = element ? {
