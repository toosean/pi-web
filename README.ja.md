...
| `PI_WEB_ALLOW_ALL_FILES` | ファイルビューアーと worktree に任意のファイルへのアクセスを許可 | 未設定 |
| `PI_WEB_IDLE_TIMEOUT_MS` | セッションのアイドルタイムアウト（ミリ秒）。`0` で無効化 | `600000`（10 分） |
| `NEXT_PUBLIC_REPLACEMENT_BASE_URL` | Markdown の localhost リンクを外部アクセス可能な URL に置換 | 未設定 |
| `PI_WEB_DIST_DIR` | ビルド出力先。稼働中の `.next` を上書きせずにビルド可能 | `.next` |
